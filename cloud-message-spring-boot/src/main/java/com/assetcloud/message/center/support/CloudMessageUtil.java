...
public class CloudMessageUtil {
    private final static Logger log = LoggerFactory.getLogger(CloudMessageUtil.class);

    public static TransactionListener convert(CloudMessageLocalTransactionListener listener) {
        return new TransactionListener() {
            @Override
            public LocalTransactionState executeLocalTransaction(Message message, Object obj) {
                CloudMessageLocalTransactionState state = listener.executeLocalTransaction(convertToSpringMessage(message), obj);
                return convertLocalTransactionState(state);
            }

            @Override
            public LocalTransactionState checkLocalTransaction(MessageExt cloudMessage) {
                CloudMessageLocalTransactionState state = listener.checkLocalTransaction(convertToSpringMessage(cloudMessage));
                return convertLocalTransactionState(state);
            }
        };
    }
    ...
    public static org.springframework.messaging.Message convertToSpringMessage(
        com.assetcloud.message.center.entity.CloudMessage message) {
        MessageBuilder messageBuilder =
            MessageBuilder.withPayload(message.getBody()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.KEYS), message.getKeys()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.TAGS), message.getTags()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.TOPIC), message.getTopic()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.MESSAGE_ID), message.getMsgId()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.BORN_TIMESTAMP), message.getBornTimestamp()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.BORN_HOST), message.getBornHostString()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.FLAG), message.getFlag()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.QUEUE_ID), message.getQueueId()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.SYS_FLAG), message.getSysFlag()).
                setHeader(toRocketHeaderKey(CloudMessageHeaders.TRANSACTION_ID), message.getTransactionId());
        addUserProperties(message.getProperties(), messageBuilder);
        return messageBuilder.build();
    }

    public static String toRocketHeaderKey(String rawKey) {
        return CloudMessageHeaders.PREFIX + rawKey;
    }
    ...
    public static org.apache.rocketmq.common.message.Message convertToRocketMessage(
        ...
        if (Objects.nonNull(headers) && !headers.isEmpty()) {
            Object keys = headers.get(CloudMessageHeaders.KEYS);
